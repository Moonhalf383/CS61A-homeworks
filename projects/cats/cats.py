...
def about(subject):
    ...
    def check(paragraph):
        ...
        for para_unit in processed_paragraph:
            for sub_unit in subject:
                if para_unit == sub_unit:
                    return True
        return False
    return check 
    # END PROBLEM 2
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0: # Feel free to remove or add additional cases
        ...
        add_diff = minimum_mewtations(typed,source[1:],limit-1)
        remove_diff = minimum_mewtations(typed[1:],source,limit-1)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[timestamps[i+1]-timestamps[i] for i in range(len(timestamps)-1)] for timestamps in timestamps_per_player]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_player = [[j for j in player_indices if min([times[k][i] for k in player_indices]) == times[j][i]] for i in word_indices]
    ans = [[words[j] for j in word_indices if [fast[0] for fast in fastest_player][j] == i]for i in player_indices]
    return ans 
    # END PROBLEM 10
