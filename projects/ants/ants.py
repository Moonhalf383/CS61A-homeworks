...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.damage_undoubled:
            self.damage_undoubled = False
            self.damage *= 2
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    implemented = True
    damage = 1
    # ADD/OVERkRIDE CLASS ATTRIBUTES HERE
    food_cost = 3
    initial_health = 1
    lower_bound = 0
    ...
    def nearest_bee(self):
        ...
        scan_place = self.place
        index = 0
        for i in range(self.lower_bound):
            if scan_place == None:
                return None 
            scan_place = scan_place.entrance
            index += 1
        while True:
            if scan_place is None or index > self.upper_bound:
                break 
            if scan_place.bees:
                if not scan_place.is_hive:
                    return random_bee(scan_place.bees)
            scan_place = scan_place.entrance
            index += 1
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained != None or other.is_container:
            return False 
        else:
            return True 
        # END Problem 8a
