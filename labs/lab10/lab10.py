...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    if isinstance(args.first, Pair) and args.first.first in OPERATORS:
        args.first = calc_eval(args.first)
    if isinstance(args.rest, Pair) and isinstance(args.rest.first, Pair):
...
def eval_define(expressions):
    ...
    """
    "*** YOUR CODE HERE ***"
    bindings[expressions.first] = expressions.rest.first if expressions.rest.first not in bindings else bindings[expressions.rest.first]
    if bindings[expressions.first] in OPERATORS:
        bindings[expressions.first] = OPERATORS[bindings[expressions.first]]
    return expressions.first 
